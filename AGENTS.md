...
* **`include/`**: All public/project header files (`*.h`) MUST be placed here.
...
sudo make install PREFIX=/usr/local  # installs c-image-viewer + assets/c-image-viewer.desktop
```
...
```
src/main.c        -> SDL init, event loop, priority: browser overlay > viewer > global
